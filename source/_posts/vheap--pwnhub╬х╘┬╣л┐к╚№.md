...
![](../img/2706180-20220514175612986-253149422.png)
...
![](../img/2706180-20220514175619263-121762209.png)
...
![](../img/2706180-20220514175624982-2031282091.png)

![](../img/2706180-20220514182146085-26007685.png)


然后输入结构体的名字

![](../img/2706180-20220514180610163-205234172.png)


将光标点到ends上，然后按d

![](../img/2706180-20220514180637568-1730011386.png)


先创建四个变量，然后将光标点到field_0上，按n，重命名

![](../img/2706180-20220514180651445-1703543613.png)


最后将光标点到函数名上，按y，然后修改第二个红色框里的内容（改成结构体的名字，我这里是更改成value）

![](../img/2706180-20220514180703104-1762048814.png)


效果如下：

![](../img/2706180-20220514180712905-1869244594.png)

还有一种方法：

#### 创建结构体 方法2：

![image-20220513155653337](../img/2706180-20220514180306157-2026482700.png)

然后右键插入

![image-20220513155758601](../img/2706180-20220514180305556-1203311905.png)

然后编辑结构体即可

![](../img/2706180-20220514180832544-593075132.png)


最后将原本的数据类型换成定义好的结构即可

![](../img/2706180-20220514180849983-205219515.png)

![](../img/2706180-20220514180901098-1038564234.png)


效果如下：

![](../img/2706180-20220514180915039-1521065964.png)


不过改完之后发现还是懵懵逼逼，因为还有个奇怪的qword_202D00不知道在干嘛。按x看一下交叉引用，发现了下面的代码

![](../img/2706180-20220514180927780-335719541.png)
...
![](../img/2706180-20220514180938745-1280833798.png)


### 存在格式化字符串漏洞

![](../img/2706180-20220514180947642-1290198064.png)
...
![](../img/2706180-20220514180959726-926685542.png)


最多输入2，接下来的循环最多跑三次，这个循环是从2020E0这里开始存一些数据。

![](../img/2706180-20220514181007362-1502659209.png)


这里输入一个不大于9的数字，然后循环会跑对应的次数，不过这里看着有点懵，不知道有啥用，那就继续往后分析。

![](../img/2706180-20220514181022124-806979207.png)
...
![image-20220513201827044](../img/2706180-20220514180304098-1657473361.png)
...
![image-20220513214802748](../img/2706180-20220514180303868-1305720818.png)

这个就是需要变量a1的第一字节和第三字节，要大于等于0 小于等于2。看下a1是什么？

一顿溯源之后，发现它就是qword_202500靠偏移得出来的值，再溯源一下，看看我们是否对这个qword_202500进行了输入

![](../img/2706180-20220514181132310-878458808.png)


下图中发现了，我们是可以控制dword_202500的值，因此这意味着我们可以控制输入的值的第一第三字节来绕过检查。

![](../img/2706180-20220514181144499-886432126.png)



![image-20220513215318327](../img/2706180-20220514180303512-502508280.png)

![](../img/2706180-20220514181203734-401889871.png)


![image-20220513215355579](../img/2706180-20220514180303249-943265363.png)
...
![](../img/2706180-20220514181218429-18861707.png)


![image-20220514090631642](../img/2706180-20220514180302997-106339106.png)
...
![](../img/2706180-20220514181236179-1811812982.png)


发现此时来到了if判断的地方，然后我们查看一下$rbp-0x24的值，发现末尾的是个什么玩意？989A68？ 这个肯定是过不了判断的。回想起现在看的是个十六进制的数字，我们用计算器转一下十进制看看。

![](../img/2706180-20220514181247254-599152087.png)


豁然开朗，**因为我们输入的是十进制类型的数据，但是最后宏来取某个字节进行判断的时候，是对十六进制的数据进行操作的**。因此为了绕过判断，我们要用十六进制的数据绕过，然后把其转换成十进制的数据输入。因此我们应该把10001000这个值改成a001000（a代表第四字节（我从低地址开始数的）设置成10为了执行malloc，第三字节的00，无所谓只要能绕过最开始的检查即可，第二字节的10代表的是malloc申请chunk的大小，第一字节的00相当于是unk_202060的索引，用来找到不同的堆块），然后转换成十进制输入。
...
![image-20220514100958393](../img/2706180-20220514180301736-97569248.png)

### sprintf的格式化字符串漏洞

我们要将fd指针修改为__free_hook的地址，获取这个地址的前提是拿到libc基地址。此时就要用到前面的格式化字符串漏洞了，先看一下sprintf函数的执行情况。

![image-20220514101654530](../img/2706180-20220514180302267-1280701485.png)
...
![](../img/2706180-20220514182802712-1937798255.png)
...
![](../img/2706180-20220514181316800-65373104.png)


![image-20220514154108694](../img/2706180-20220514180301381-730330410.png)
...
![image-20220514155318872](../img/2706180-20220514180300891-657482779.png)
